...
interface BlogHistoryPageProps {
  articles: Article[]
...
export function BlogHistoryPage({ articles, progress, statusFilter }: BlogHistoryPageProps) {
  ...
  const handleFilterChange = (newFilter: HistoryStatusFilter) => {
    ...
    const newUrl = params.toString() ? `/blog/history?${params.toString()}` : '/blog/history'
    router.push(newUrl)
  }

  // Grid classes based on view mode
  const gridClasses = cn(
    viewMode === 'featured' && 'grid grid-cols-1 md:grid-cols-2 gap-6 auto-rows-fr',
