...
/**
 * Code block component for Lexical rich text with Shiki syntax highlighting.
 * Matches the styling from CodeBlock.tsx used in markdown rendering.
 *
 * Security: HTML is sanitized with DOMPurify before rendering.
 */
export function LexicalCodeBlock({ code, language }: LexicalCodeBlockProps) {
  ...
  useEffect(() => {
    ...
    async function highlight() {
      try {
        ...
        // Sanitize the HTML output for security (XSS protection)
  ...
  return (
    <div className="my-6 overflow-hidden rounded-lg border border-rga-green/20 bg-[#1a1b26]">
      ...
      {/* Code content - HTML is sanitized with DOMPurify before rendering */}
      <div className="overflow-x-auto">
        {highlightedCode ? (
          <div
            className="p-4 text-sm font-mono leading-relaxed [&>pre]:!bg-transparent [&>pre]:!p-0 [&>pre]:!m-0"
            // Safe: HTML is sanitized above with DOMPurify before being set
