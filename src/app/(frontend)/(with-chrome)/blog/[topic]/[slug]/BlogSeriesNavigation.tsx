...
export function BlogSeriesNavigation({ navigation, seriesProgress, isAuthenticated = false }: BlogSeriesNavigationProps) {
  const { seriesName, seriesSlug, currentOrder, totalParts, articleIds, previous, next } = navigation
  const seriesUrl = getSeriesUrl({ slug: seriesSlug })
  ...
  return (
    ...
    >
      ...
      {/* Progress dots - only show for authenticated users with progress */}
      {isAuthenticated && (
